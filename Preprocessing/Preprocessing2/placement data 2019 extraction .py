...
for file in file_names:
    abc = camelot.read_pdf('2019/'+file, pages="all",flavor='lattice',line_scale=30) # file loation
...
final.to_csv('./Outputs/placement2019.csv', index=False)
...
file_names = []
for i in os.listdir(r'2019/failed colleges'):
    # print(i)
    file_names.append(i)
file_names
...
k = 0
for file in file_names:
    abc = camelot.read_pdf('2019/failed colleges/'+file, pages="all",flavor='lattice',line_scale=30) # file loation
    # line_space = 30 , this argument helps to read single row tables also. greater the value, higher the chances of detecting smaller tables.
    pdf_file = open('2019/failed colleges/'+file, 'rb')
    read_pdf = PyPDF2.PdfFileReader(pdf_file)
    number_of_pages = read_pdf.getNumPages()
    page = read_pdf.getPage(0)
    page_content = page.extractText()
...
final.to_csv('./Outputs/placement2019_different_type_of_data.csv',index=False)
